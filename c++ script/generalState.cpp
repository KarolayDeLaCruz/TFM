...
enum estados {START, EXPLORE, WAIT, GROOM, ATTENTION, SLEEP };
//6 Posturas 
enum posture {NEUTRAL, HEADLEFT, HEADRIGHT, HAPPY, FEAR, AGGRESIVE };

generalState::generalState(void)
{
	//Inicializa con 0's matriz umbral reacciones
	//Nstates=8
	...
	//Inicializa con 0's vectores: umbral sensores, siguiente estado, estado actual
	//Nsensor=11
...
generalState::generalState(vector<int> current, vector<int> next, vector<int> sensorThres, vector<vector<double>> thresReac)
{
	...
	//Asigna thresReac a thresholdReaction
	//Nstates=8
	for ( size_t i = 0; i < Nstates; i++)
	{
		for (size_t j = 0; j < (Nstates); j++)
	...
	//Asigna sensorThres a sensorThreshold
	for (size_t i = 0; i < Nsensor; i++)
	{
...
vector<int> generalState::checkGuardCondition(vector<vector<double>> currentReaction, vector<int> nextStat)
{
	...
	cout << currentReaction[previousState][nextState]  << endl;
	cout << generalState::thresholdReaction[previousState][nextState] << endl;
	...
	{
		cout << "no se cumple condicion" << endl;
...
void generalState::checkTransitionState(vector<int> sensorReading, vector<vector<double>> currentReaction, vector<vector<double>> initialReaction)
{
	...
	{
		//diffsensor= abs(lectura sensor - umbral sensor) 
		diffSensor.push_back(abs(sensorReading[i] - generalState::sensorThreshold[i]));
	}

	//indice de la mayor diferencia entre sensores 
	size_t maxElementIndex = max_element(diffSensor.begin(), diffSensor.end()) - diffSensor.begin(); 
	...
	case START:
		cout << "karo";
		printVector(guarCondResult);

		if (diffSensor[maxElementIndex] <= 0)		 //no sensor
		{
			generalState::setNextState({ START,0 }); //estado -> explorar
		}
		else
		{
...
vector<vector<double>> generalState::entryActivity(vector<int> state, vector<vector<double>> currentReaction, vector<vector<double>> initialReaction)
{
	...
	case EXPLORE:
		...
		for (size_t j = 0; j < currentReaction[0].size(); ++j)
		{
			...
			currentReaction[3][j] += 0.1;
		}
		break;
	case ATTENTION:
		//aumentar desconfianza
		cout << "Attention!" << endl;
		for (size_t j = 0; j < currentReaction[5].size(); ++j)
		{
